...
- **Card Master**: システムが管理するカードの一覧。ユーザーはこの一覧からカードを選択してコレクションに追加する
...
1. WHEN ユーザーがカードコレクション画面にアクセスする、THE System SHALL システムが提供するカードマスター一覧を表示する
2. WHEN ユーザーが欲しいカードコレクションにカードを追加する、THE System SHALL カードマスター一覧から選択されたカード情報を保存しユーザーアカウントに関連付ける
3. WHEN ユーザーが出せるカードコレクションにカードを追加する、THE System SHALL カードマスター一覧から選択されたカード情報を保存しユーザーアカウントに関連付ける
4. WHEN ユーザーがいずれかのコレクションからカードを削除する、THE System SHALL データ整合性を維持しながら関連付けを削除する
5. WHEN ユーザーが自分のカードコレクションを表示する、THE System SHALL 欲しいカードと出せるカードの両方のコレクションにあるすべてのカードを表示する
6. WHEN ユーザーがいずれかのコレクションのカード数量を更新する、THE System SHALL 保存されている数量値を更新する
7. WHEN ユーザーがカードマスターに存在しないカードを追加しようとする、THE System SHALL エラーを表示し追加を拒否する
